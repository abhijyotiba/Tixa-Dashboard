...
export default function ChatPage() {
  ...
  useEffect(() => {
    async function fetchUrlAndToken() {
      try {
        ...
        // Get the configured Agent Console URL
        const url = await getAgentConsoleUrl();
        setAgentConsoleUrl(url);
        
        // If we have both URL and token, create authenticated URL
  ...
  // No URL configured - show setup message
  if (!agentConsoleUrl) {
    return (
      <div className="flex h-screen bg-gray-50">
        <Sidebar />
